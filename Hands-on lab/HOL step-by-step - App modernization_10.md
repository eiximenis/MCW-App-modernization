...
5. Enter the following command at the command prompt. The tokenized values should be replaced as follows:
   ...
   - `[FILE-SOURCE]`: This is the path to the `policy-documents` folder your downloaded copy of the GitHub repo. If you used the extraction path of `C:\MCW`, the path is `C:\MCW\MCW-App-modernization-prod\Hands-on lab\lab-files\policy-documents`.
...
6. The final command should resemble the following:
   ...
   ```bash
   AzCopy /Source:"C:\MCW\MCW-App-modernization-prod\Hands-on lab\lab-files\policy-documents" /Dest:"https://contoso281470.blob.core.windows.net/policies" /DestKey:"XJT3us2KT1WQHAQBbeotrRCWQLZayFDNmhLHt3vl2miKOHeXasB7IUlw2+y4afH6R/03wbTiRK9SRqGXt9JVqQ==" /S
   ```
